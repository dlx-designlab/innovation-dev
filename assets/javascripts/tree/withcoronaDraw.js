...
import data from "../../data/withcorona-en.json";
...
const draw = () => {
  ...
  svg.attr("viewBox", [0, 0, width, height]).style("font", "12px sans-serif");
    ...
    for (const tag of c.data.tags) {
      const _tag = target
        ...
        .on("click", function (e, d) {
          ...
          location.href = `./cards.html?tag=${this.dataset.tag}`;
        });
      tags.push(_tag);
  ...
  cell.each((c, i, nodes) => {
    ...
    const description = target
      ...
      .on("click", function () {
        // TODO: リンクにパラメータ入れる
        location.href =
          "./cards.html?card_id=" + c.data.card_id + "#" + c.data.card_id;
      });
    let cnt = 0;
    const lineHeight = 14;
